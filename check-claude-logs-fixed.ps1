# Claude for Desktop ログ確認スクリプト - 修正版
...
$processes = Get-Process | Where-Object { $_.ProcessName -like "*claude*" -or $_.ProcessName -like "*node*" }
if ($processes) {
    $processes | Select-Object ProcessName, Id, StartTime, CPU | Format-Table
} else {
    Write-Host "Claude/Node関連のプロセスは見つかりませんでした" -ForegroundColor Yellow
}
